...
class ItemLike{
    ...
    fun enchantWithLevels(levels: NumberProvider, treasure: Boolean){
        ...
        minPoints += 1
        maxPoints += 1 + (stack.item.enchantmentValue / 4) * 2
        ...
        for (ench in BuiltInRegistries.ENCHANTMENT){
            if ((ench.isTreasureOnly && !treasure)
                || !ench.isDiscoverable
                || (!stack.`is`(Items.BOOK) && (!ench.isPrimaryItem(stack) || !ench.canEnchant(stack))))
                continue
            for (lvl in ench.minLevel..ench.maxLevel){
    ...
    fun enchantRandom(enchants: HolderSet<Enchantment>){
        tryEnchantedBook()
        for (enchHolder in enchants){
            val ench = enchHolder.value()
            for (lvl in ench.minLevel..ench.maxLevel)
                enchantments.add(EnchantmentInstance(ench, lvl))
        }
    }
    ...
    fun tryEnchantedBook(){
        if (stack.item is BookItem) stack = ItemStack(Items.ENCHANTED_BOOK).apply{applyComponents(stack.components)}
    }

    fun writeMap(){
        if (stack.item is MapItem) stack = ItemStack(Items.FILLED_MAP).apply{applyComponents(stack.components)}
        wrapHoverName(stack)
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        other as ItemLike
        if (!ItemStack.isSameItemSameComponents(stack, other.stack))
            return false
